<%
ordered = Session.Contents("Ordered")
if ordered = "true"then
    Session.Contents.Remove("Ordered")
    call OrderSuccess
end if 
                ...
                <div class="col-lg-8">
                                ...
                                <tr><% 
                                    ...
                                    <td class="cart__price"><%Response.write("₹"&indiProductPrice(i))%></td>
                                      <td class="cart__close">
                                        <form method ="post" action="http://localhost/WtProject/cake/cake/cancel.asp">
                                        <input type = "hidden"name ="indexCancel"  value=<%response.write(i+1)%>>
                                        <a onClick = "this.parentNode.submit();"><span class="icon_close"></span></a>
                                        </form>
                 ...
                 <div class="col-lg-4">
                    <div class="cart__total">
                        <h6>Your Address</h6>
                        <ul><%dim cookieThere,bool
                        bool=0
                        cookieThere = 0%>
                            <li> <img src="img/location.png" width="35px"> 
                                <%if Session.Contents("Address") = "true" then
                                cookieThere = 1
                                Session.Contents("Address") = "false"
                                 Response.Cookies("add1") = Request.form("add1")
                                  Response.Cookies("add2") = Request.form("add2")
                                   Response.Cookies("add3") = Request.form("add3")
                                Response.Cookies("add4") = Request.form("add4")
                                Response.write(Request.form("add1")&","&Request.form("add2")&","&Request.form("add3")&","&Request.form("add4"))%></li><%
                            else 
                            for each x in Request.Cookies
                             cookieThere = 1
                                  if bool=0 then
                                bool = 1
                             Response.write(Request.Cookies("add1")&","&Request.Cookies("add2")&","&Request.Cookies("add3")&","&Request.Cookies("add4"))%></li><%
                            end if
                            next
                        end if
                                if cookieThere = 0 then
                                Response.write("Mumbai")
                                end if%>
                        </ul>
                        <a href="Address.asp" class="primary-btn">Edit Address</a>
                    </div>
                     <!-- <div class="col-lg-4"> -->
                    <div class="cart__total">
                        <h6>Cart total</h6>
                        <ul>
                            <% 
                            for i=0 to itemNo-1%>
                             <li><% response.write(allProduct(i))%><span><% response.write("₹"&indiProductPrice(i))%></span></li>
                            <%next%>
                            <li>Total <span><%response.write("$"&TotalPrice)%></span></li>
                        </ul>
                        <a onclick="return Ordered();"href="Order.asp" class="primary-btn">Place Order</a>
                    </div>
               <!--  </div>  -->
                </div>
